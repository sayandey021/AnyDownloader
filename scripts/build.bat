...
echo Closing any running instances to avoid file locks...
taskkill /F /IM flet.exe /T >nul 2>&1
taskkill /F /IM flet_bin.exe /T >nul 2>&1
taskkill /F /IM AnyDownloaderApp.exe /T >nul 2>&1
...
python custom_pack.py ..\main.py -y --name "AnyDownloaderApp" --icon ..\assets\icon.ico --add-data "..\assets;assets" --add-data "%YTMUSIC_LOCALES%;ytmusicapi/locales" --distpath "..\dist" --product-name "Any Downloader" --file-description "Any Downloader" --product-version "1.9.4.0" --file-version "1.9.4.0" --company-name "SwiftGrab" --copyright "Copyright (c) 2026 SwiftGrab"
